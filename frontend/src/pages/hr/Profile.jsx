...
export default function HRProfile() {
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const [profileRes, employeesRes, internsRes] = await Promise.all([
      ...
      setCounts({
        ...
        interns: internsRes.data?.pagination?.total || 0,
      });
      ...
      toast.error('Failed to load HR profile');
    } finally {
      setLoading(false);
    }
  };
  ...
  const relations = useMemo(() => ([
    ...
    { label: 'HR Team Scope', value: `${counts.employees} employees / ${counts.interns} interns` },
    { label: 'Department', value: profile?.department?.name || '-' },
    { label: 'Role', value: profile?.role?.name || 'HR Manager' },
  ]), [counts.employees, counts.interns, profile]);
    ...
    <PageWrapper>
      <div className="max-w-6xl mx-auto space-y-6 pb-10 font-sans">
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm">
          <h1 className="text-2xl font-bold text-[#0F172A]">HR Profile</h1>
          <p className="text-sm text-[#64748B] mt-1">HR is now interconnected with employees, interns, and organizational roles.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm lg:col-span-2">
            <h2 className="text-sm font-semibold text-[#0F172A] mb-4">Interconnected Scope</h2>
