...
import { 
  ...
  X, Info, UploadCloud, ChevronDown, ChevronUp 
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// --- MOCK DATA ---
// BACKEND: GET /api/intern/leave/balance
const mockLeaveBalance = {
  casual: { total: 5, used: 1, remaining: 4 },
  sick: { total: 3, used: 0, remaining: 3 },
  emergency: { total: 2, used: 0, remaining: 2 }
};

// BACKEND: GET /api/intern/leave/requests
const mockLeaveRequests = [
  { 
    _id: "leave001", type: "Casual", fromDate: "2024-10-20", toDate: "2024-10-21", 
    days: 2, reason: "Family function", status: "Pending", 
    reviewedBy: null, appliedAt: "2024-10-12T10:00:00Z" 
  },
  { 
    _id: "leave002", type: "Sick", fromDate: "2024-09-15", toDate: "2024-09-15", 
    days: 1, reason: "Fever and cold", status: "Approved", 
  ...
  { 
    ...
    reviewedBy: { name: "Sarah Johnson" }, reviewNote: "Insufficient leave balance at the time of application", 
...
const ApplyLeaveModal = ({ isOpen, onClose, hrName, onSubmit }) => {
  ...
  const getBalance = (leaveType) => {
    ...
    if (leaveType === 'Emergency') return mockLeaveBalance.emergency.remaining;
    return 0;
  };
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                ...
                { id: 'Emergency', icon: AlertCircle, color: 'text-[#D97706]', activeBg: 'bg-[#FFFBEB]', border: 'border-[#D97706]' }
              ].map(opt => {
                const Icon = opt.icon;
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm font-bold ${isActive ? 'text-[#0F172A]' : 'text-[#64748B]'}`}>{opt.id}</span>
                  </button>
                )
              })}
            </div>
      ...
      <div className="w-full flex flex-col gap-6 max-w-[1200px] mx-auto pb-10 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start mb-1">
                <span className="text-xs font-bold text-[#64748B] uppercase tracking-wider">Casual Leave</span>
              ...
              <p className="text-[10px] font-bold text-[#64748B] text-right">{mockLeaveBalance.casual.used} of {mockLeaveBalance.casual.total} days used</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start mb-1">
                <span className="text-xs font-bold text-[#64748B] uppercase tracking-wider">Sick Leave</span>
              ...
              <p className="text-[10px] font-bold text-[#64748B] text-right">{mockLeaveBalance.sick.used} of {mockLeaveBalance.sick.total} days used</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start mb-1">
                <span className="text-xs font-bold text-[#64748B] uppercase tracking-wider">Emergency Leave</span>
                <span className="text-xl font-black text-[#0F172A]">{mockLeaveBalance.emergency.remaining}</span>
              </div>
              <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden mb-1">
                <div className="h-full bg-[#D97706]" style={{ width: `${(mockLeaveBalance.emergency.used/mockLeaveBalance.emergency.total)*100}%` }} />
              </div>
              <p className="text-[10px] font-bold text-[#64748B] text-right">{mockLeaveBalance.emergency.used} of {mockLeaveBalance.emergency.total} days used</p>
            </div>
          </div>
